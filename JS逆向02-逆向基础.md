...
2.如果抠下来的代码放在Pycharm当中运行卡死，有两个可能性：环境检测、格式化检测。如果是格式化检测通常会用到"toString"方法，就新打开一个浏览器标签将代码放入Console里，在第一行写上debugger再运行一步步调试，直到卡死的那一行代码。
3.当我们抠出了所有的JS代码时，最好放在一个大函数内，最后用return返回结果。
