...
| --------------------------------------------------------- | -------- | -------------------- | ------------------------------------------------------------ |
|  |  |  |  |
| [JS逆向03-简单访问逻辑](JS逆向03-简单访问逻辑.md)         | 非常简单 | 访问逻辑，请求头顺序 | 一是访问页面的逻辑；二是请求头的顺序需要使用Fiddler展示的。  |
| [JS逆向04-简单GET参数加密](JS逆向04-简单GET参数加密.md) | 非常简单 | JS加密（GET参数） | 一是通过Initiator里的request定位发送请求的代码；二是百度得到 `btoa()` 方法JS源码，返回base64编码。 |
