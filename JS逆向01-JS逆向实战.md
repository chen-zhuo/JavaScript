...
![QQ截图20220203002142](image/QQ截图20220203002142.png)

![QQ截图20220203002301](image/QQ截图20220203002301.png)

?> 提示：用相同的名称重写图中的 `toString()` 方法，当js引擎向上查找的时候，就使用的重写方法，而不是使用的 `Object.toString()` 方法，以此来达到以假乱真的目的。
...
**函数是Function类型的实例对象，所以函数本身拥有的对象属性，可以简单的理解：函数即对象。**函数的 `__proto__`  指向 `Function.prototype ƒ() {[native code]}` 内容：

![QQ截图20220203005718](image/QQ截图20220203005718.png)

用 new 关键字来调用的函数时，过程如下：

**(1)  每当使用 new 关键字调用函数时，就会开辟一个新的内存空间，用来存储该函数实例化的对象；**
**(2)  函数体内部的 this 指向该内存空间，给 this 添加属性，就相当于给实例添加属性。**

![QQ截图20220203012815](image/QQ截图20220203012815.png)

?> 提示：如果不写new，那么p1就等于Person的返回值，即 `undefined` 。

p1是Person函数的实例对象，也就是说Person是p1的构造函数。按照上面原型链的解释，就会有**如同上面的函数的等价关系**：
...
**函数表达式中的函数则不会出现函数提升，而是解释器逐行解释，因此如果调用在函数表达式之前，则会调用失败。**如下
...
比如我有个构造函数A，然后又有个构造函数B，但是B想要使用A里的一些属性和方法，一种办法就是让我们自身化身为CV侠，复制粘贴一波。还有一种就是利用继承，我让B直接继承了A里的功能，这样我就能用它了。这里我们讲解原型链继承：

**原型链继承：就是将要继承构造函数的实例化对象赋值给被继承函数的 `prototype` 属性。**
...
1. 默认绑定(非严格模式下this指向全局对象window，严格模式下this会绑定到undefined)；
...
3. 显示绑定(通过 `call()` 或者 `apply()` 方法直接指定this的绑定对象)，需要注意的是使用 `call()` 或者 `apply()` 的函数是会直接执行的，`call()` 接收若干参数 `fn.call(this, 1, 2, 3)`， `apply()` 接收的是一个数组 `fn.apply(this,[1, 2, 3])`，`bind()` 是返回值为一个新的函数，需要再次调用 `fn.bind(this)(1, 2, 3)` 才会执行。如果call、apply、bind接受到的第一个参数是空或者null、undefined的话，则会忽略这个参数。另外，forEach、map、filter函数的第二个参数也是能显式绑定this的。

![QQ截图20220205033859](image/QQ截图20220205033859.png)

4. new绑定。下图中重新定义了name，因此后面 `this.name` 输出了123456，**但即使刷新清理缓存后也还是会输出123456，除非重新打开一个新的选项卡，输出才会为空，这是由于浏览器的特性，不清理window的name属性。**
...
5. 箭头函数绑定(**this的指向由外层作用域决定的**)，ES6标准新增了一种新的函数：Arrow Function（箭头函数）。因为它的定义用的就是一个箭头。
...
1. 函数当中里面定义的变量，由于作用域的影响是不会绑定到window，即使在闭包函数中也是同样的效果中；

![QQ截图20220205025807](image/QQ截图20220205025807.png)

2. 使用 `let`、`const` 等关键字是不会与this绑定的；

![QQ截图20220205023921](image/QQ截图20220205023921.png)

3. 被隐式绑定的函数在特定的情况下会丢失绑定对象，即隐式丢失。**一种是使用另一个变量来给函数取别名，另一种是将函数作为参数传递时会被隐式赋值，回调函数丢失this绑定。**下图中 `obj.foo()` 中的this指向调用者 `obj`；`foo2()` 发生了隐式丢失，调用者是window，使得 `foo()` 中的 `this` 指向window；`foo3()`发生了隐式丢失，调用者是 `obj2`，使用 `foo()` 中的 `this` 指向 `obj2`；

![QQ截图20220205031607](image/QQ截图20220205031607.png)

4. 把一个函数当成参数传递到另一个函数的时候，也会发生隐式丢失的问题，且与包裹着它的函数的this指向无关。在非严格模式下，会把该函数的this绑定到window上，严格模式下this会绑定到undefined；

![QQ截图20220205032652](image/QQ截图20220205032652.png)

5. **this永远指向最后调用它的对象。**下图中，有一个setTimeout定时器实质就是 `window.setTimeot` 当中的方法，因此this也就指向了window。

![QQ截图20220205034508](image/QQ截图20220205034508.png)

但我们可以通过 `call()` 或者 `apply()` 方法直接指定this的绑定对象。下图就通过 `call()` 绑定了obj1，**需要注意的是 `obj2.foo2().call(obj1)` 这种写法改变的就是foo2函数内的this指向，和setTimeout里面的this是没有关系的，因为调用定时器的始终是window**：
